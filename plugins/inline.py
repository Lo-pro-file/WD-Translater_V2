...
async def inline(_, query: InlineQuery):
        ...
        if string == "":
        	METHOD=  [ InlineQueryResultArticle( title="❣️ How To Use ❣️", input_message_content=InputTextMessageContent("Ex ; ```@Translate_v3_bot how to use # hi```"),description="{Text} # {language code} ",thumb_url="https://tg-link.herokuapp.com/dl/0/AgADh60xG50-wFc.jpg")]
        ...
        try:
        	await query.answer(results=TRTEXT ,  cache_time=2, switch_pm_text="WD Google Translater",switch_pm_parameter="start")
        except:
        	return
